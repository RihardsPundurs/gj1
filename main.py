...
def collision_sprite():
    global last_surface
    total_list = []
    for list_object in pygame.sprite.spritecollide(player.sprite, level_groups,
                                                   False):
    ...
    for entityf in funny_list:
        if entityf in blocks.sprites() or entityf in platforms.sprites(
        ) or entityf in platforms_disappearing.sprites(
        ):  #and not entity in platforms.sprites() and not entity in platforms_disappearing.sprites()
            player.sprite.on_ground = True
...
checkpoints.add(Checkpoint([0, 0], 0))
...
while True:
    ...
    if game_active and not win_bool:
        ...
        username_text_rect = username_text_surf.get_rect(center=(640, 360))
        timer_text_surf = test_font.render(
            str(round(end_time - start_time, 2)), False, "Black")
        timer_text_rect = timer_text_surf.get_rect(topleft=(60, 60))
        screen.blit(timer_text_surf, timer_text_rect)
